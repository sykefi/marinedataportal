...
describe('Water quality module', () => {
  ...
  it('has error if depth start is greater than depth end', () => {
    module.selectedDepth = { option: DepthOptions.DepthInterval, start: 1, end: 2 };
    expect(module.errors).not.contains('$depthStartGreaterThanDepthEnd');

    module.selectedDepth = { option: DepthOptions.DepthInterval, start: 2, end: 2 };
    expect(module.errors).not.contains('$depthStartGreaterThanDepthEnd');

    module.selectedDepth = { option: DepthOptions.DepthInterval, start: 3, end: 2 };
    expect(module.errors).contains('$depthStartGreaterThanDepthEnd');

    // check that values are ignored when option is not interval
    module.selectedDepth = { option: DepthOptions.SurfaceLayer, start: 3, end: 2 };
    expect(module.errors).not.contains('$depthStartGreaterThanDepthEnd');
